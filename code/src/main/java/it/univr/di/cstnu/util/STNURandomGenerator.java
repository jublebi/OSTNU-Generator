...
//	static final Class<STNU> STNU_CLASS = it.univr.di.cstnu.algorithms.STNU.class;
	...
	public static void main(String[] args) throws FileNotFoundException, IOException {
		final STNURandomGenerator generator = new STNURandomGenerator();
		...
		ObjectPair<TNGraph<STNUEdge>> instances;
		...
		for (int dcInstancesDone = 0; dcInstancesDone < generator.dcInstances; dcInstancesDone++) {
			...
			if (generator.dense) {
				...
				final TNGraph<STNUEdge> denseInstance = generator.makeDenseInstance(instances.getFirst());
			...
			if (notDCinstancesDone < generator.notDCInstances) {
				...
				if (generator.dense) {
					// make the dense version
					final TNGraph<STNUEdge> denseInstance = generator.makeDenseInstance(instances.getSecond());
	...
	private static int addRedundantNewEdges(
		LabeledNode source, LabeledNode dest, int weight, int hop, int requiredEdges, TNGraph<STNUEdge> denseDc) {
		if (requiredEdges <= 0) {
			return 0;
		}
		if (hop == 0) {
			final STNUEdge e = denseDc.findEdge(source, dest);
			if (e == null) {
				final STNUEdge newEdge = denseDc.getEdgeFactory().get(source.getName() + "-" + dest.getName());
		...
		int rAdd;
		for (final STNUEdge edge : denseDc.getOutEdges(dest)) {
...
//	@SuppressWarnings("UnstableApiUsage")
	 ...
	 */
	static private void stnuPlainWriter(TNGraph<STNUEdge> graph, File outputFile) {
...
//			Collections.shuffle(randomVertexList);
			...
			final ObjectArrayList<STNUEdge> randomContingentList = new ObjectArrayList<>();
			for (final STNUEdge edge : graph.getEdges()) {
...
//			Collections.shuffle(randomContingentList);
			for (final STNUEdge e : randomContingentList) {
				final LabeledNode activation = graph.getSource(e);
				final LabeledNode contingent = graph.getDest(e);
				final STNUEdge lower = graph.findEdge(contingent, activation);
				assert lower != null;
	...
	/**
	 * Subdirectory containing not DC instances
	 */
	...
	private void addForwardOrBackwardEdge(
		int firstNodeIndex, int secondNodeIndex, TNGraph<STNUEdge> g, EdgeSupplier<STNUEdge> edgeFactory,
		...
		final STNUEdge forwardEdge;
		final STNUEdge backEdge;
		...
		if (isEdgeToAdd) {
			...
			if (forwardWeight < 0) {
				...
				forwardWeight /=
					1.5;//If negative, we make it less negative otherwise it is difficult to have DC property
			}
		...
		// we consider the possibility to add a backwardEdge
...
//		}
	...
	private STNUEdge addRndEdge(
		...
		final STNUEdge e = edgeFactory.get(firstNode.getName() + "-" + secondNode.getName());
	...
	private ObjectPair<TNGraph<STNUEdge>> buildAPairRndTNInstances(boolean alsoNotDcInstance) {
		...
		final TNGraph<STNUEdge> randomGraph = new TNGraph<>("", EdgeSupplier.DEFAULT_STNU_EDGE_CLASS);
		TNGraph<STNUEdge> notDCGraph = null;
//		LabeledNodeSupplier nodeFactory = randomGraph.getNodeFactory();
		final EdgeSupplier<STNUEdge> edgeFactory = randomGraph.getEdgeFactory();
		...
		final STNU stnu = new STNU(randomGraph, timeOut);
		stnu.setDefaultControllabilityCheckAlg(CheckAlgorithm.FD_STNU);
	...
	private void createReadmeFiles(STNURandomGenerator this) throws IOException {
		...
		String readmeText = "(O)STNU RANDOM INSTANCE BENCHMARK\n" + "==============================\n\n" +
		                    ...
		                    "max out-degree:\t\t" + String.format("%4d%n", Integer.valueOf(outDegree)) +
		                    "max in-degree:\t\t" + String.format("%4d%n", Integer.valueOf(inDegree)) +
		                    "edge Probability:\t" + String.format("%4.2f%n", Double.valueOf(edgeProb)) +
	...
	private LabeledNode[] generateNodesAndContingentLinks(
		TNGraph<STNUEdge> randomGraph, EdgeSupplier<STNUEdge> edgeFactory) {
		...
		for (int slot = 1; slot <= nSlots; slot++) {
			...
			final LabeledNode ctgNode = LabeledNodeSupplier.get("C%d".formatted(indexLastAddedCtg));
	 ...
	 */
	private STNUEdge[] generateRandomEdges(
		TNGraph<STNUEdge> randomGraph, LabeledNode[] addedNodes, EdgeSupplier<STNUEdge> edgeFactory) {
...
//				final String dest = addedNodes[j].getName().substring(0, 1);
		...
		return addedEdges.toArray(new STNUEdge[0]);
	}
	 ...
	 * @return the array of random edges added.
	 */
	private STNUEdge[] generateRandomLanes(
		TNGraph<STNUEdge> randomGraph, LabeledNode[] addedNodes, EdgeSupplier<STNUEdge> edgeFactory) {
	...
	private STNUEdge[] generateRandomTree(
		TNGraph<STNUEdge> randomGraph, LabeledNode[] addedNodes, EdgeSupplier<STNUEdge> edgeFactory) {
	...
	private TNGraph<STNUEdge> makeDenseInstance(TNGraph<STNUEdge> instance) {
		...
		final TNGraph<STNUEdge> denseI = new TNGraph<>(instance, instance.getEdgeImplClass());
